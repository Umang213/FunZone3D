...
public static class ETCMenu
{
	...
	static void AddInputManager () => ETCInput.Instance.Create ();
	...
	static GameObject SetupUI ()
	{
		...
		// Event system
		if (Object.FindObjectOfType (typeof (EventSystem)) == null) AddEventSystem ();

		return canvas;
	}

	static void AddEventSystem () => new GameObject ("EventSystem", typeof (EventSystem), typeof (StandaloneInputModule));

	static GameObject AddCanvas (bool isSpaceCamera = false)
	{
		GameObject canvas = new GameObject ("MainCanvas", typeof (RectTransform), typeof (Canvas), typeof (CanvasScaler), typeof (GraphicRaycaster));
		if (isSpaceCamera)
		{
			canvas.GetComponent<Canvas> ().renderMode = RenderMode.ScreenSpaceCamera;
			canvas.GetComponent<Canvas> ().worldCamera = Camera.main;
		}
		else canvas.GetComponent<Canvas> ().renderMode = RenderMode.ScreenSpaceOverlay;
